...
class AwsPlatform extends AutoScaleCore.CloudPlatform {
    ...
    /** @override */
    async updateRouteTable(routeTableId, route) {
        logger.info('calling updateRouteTable');

        let params = {
            RouteTableId: routeTableId
        };
        if (!routeTableId) {
            logger.error(`error: unknown route table id: ${routeTableId}`);
            params = null;
        } else if (!route.destinationCidr) {
            logger.error(`error: unknown destination: ${route.destinationCidr}`);
            params = null;
        } else {
            if (route.ipv6) {
                params.DestinationIpv6CidrBlock = route.destinationCidr;
            } else {
                params.DestinationCidrBlock = route.destinationCidr;
            }
            switch (route.targetType) {
                case 'instance':
                    params.InstanceId = route.instanceId;
                    break;
                case 'nic':
                    params.NetworkInterfaceId = route.networkInterfaceId;
                    break;
                default:
                    logger.error(`error: unknown target type: ${route.targetType}`);
                    params = null;
                    break;
            }
        }

        // try replace-route first, if route not exists, it generates an error, then run
        // create-route
        let done = false,
            retry = false;
        logger.info('route to update: ', JSON.stringify(params));
        try {
            await ec2.replaceRoute(params).promise();
            logger.info(
                `existing route ${route.destinationCidr} ` +
                    `is updated on table (id: ${routeTableId}).`
            );
            done = true;
        } catch (error) {
            if (error && error.statusCode === 400 && error.code === 'InvalidParameterValue') {
                retry = true;
            } else {
                logger.error(error);
            }
        ...
        if (retry) {
            try {
                ...
                logger.info(
                    `new route ${route.destinationCidr} ` +
                        `is created on table (id: ${routeTableId}).`
                );
                done = true;
            } catch (error) {
                logger.error(
                    `cannot update route on route table id: ${routeTableId}` + `, error: ${error}`
                );
            }
        }
        logger.info('called updateRouteTable');
        return done;
    }
...
class AwsAutoscaleHandler extends AutoScaleCore.AutoscaleHandler {
    ...
    async getFazIp() {
        try {
            const registeredFAZ =
                this._settings.fortianalyzer && JSON.parse(this._settings.fortianalyzer);
            return await Promise.resolve(registeredFAZ.ip);
        } catch (error) {
            logger.error('failed to run getFazIp(). Error occurred:');
            logger.error(error);
            return false;
        }
    }
    ...
    async authorizeDevice() {
        let event = {
            ResourceProperties: {
                ServiceType: 'authorizeFgt'
            }
        };
        lambda.invoke(
            {
                FunctionName: this._settings['faz-handler-name'],
    ...
    /** @override */
    async updateRouteForEgressNATGateway(instance) {
        logger.info('calling updateRouteForEgressNATGateway');
        const routeTableId = this._settings['egress-traffic-route-table'];
        if (!routeTableId) {
            logger.error(`invalid route table id: ${routeTableId}`);
        } else {
            let hasSecondNic = this._settings['enable-second-nic'] === 'true';
            let networkInterfaceId;
            if (hasSecondNic) {
                const networkInterface = instance.networkInterfaces.find(nic => {
                    return nic.Attachment.DeviceIndex === 1;
                });
            ...
            let route = {
                ...
                targetType: (hasSecondNic && 'nic') || 'instance'
            };
            await this.platform.updateRouteTable(routeTableId, route);
        }
        logger.info('called updateRouteForEgressNATGateway');
    }
