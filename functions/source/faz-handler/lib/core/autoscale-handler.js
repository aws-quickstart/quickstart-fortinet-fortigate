...
module.exports = class AutoscaleHandler {
        ...
        if (
            ...
            if (
                ...
                if (!(await this.platform.finalizeMasterElection())) {
                    ...
                    // master election is finalized and now I am the new master
                    this._masterRecord = null;
                    await this.retrieveMaster();
                    // update tags
    ...
    /**
     * Process the device authorization. Intended to be done via FortiAnalyzer Device Manager.
     * If FortiAnalyzer is not supported in a version, don't need to touch this function.
     * To support it for a platform, override this function in the platform-specific
     * autoscale-handler class.
    ...
    /**
     * update the route tables for private subnet which need a NAT Gateway for egress traffic.
     * it should update the route on each subnet with src:dest to 0.0.0.0/0:master fgt in asg
     * @param {Object} instance the FortiGate instance to be the new NATGateway
     */
    // eslint-disable-next-line no-unused-vars
