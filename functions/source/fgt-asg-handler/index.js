...
const logger = new fgtAutoscaleAws.AutoScaleCore.DefaultLogger(console);
const autoscaleHandler = new fgtAutoscaleAws.AwsAutoscaleHandler();
if (
...
exports.AutoscaleHandler = async (event, context, callback) => {
    ...
    await fgtAutoscaleAws.handler(event, context, callback);
};
...
exports.ByolLicenseHandler = async (event, context, callback) => {
    console.log(`Incoming event: ${JSON.stringify(event)}`);
    await fgtAutoscaleAws.handleGetLicense(event, context, callback);
};
...
async function restart() {
    ...
    // delete the primary election result
    ...
    // bring up the 1st instance which will become the primary instance
    if (
        ...
        if (autoscaleHandler._settings['primary-election-no-wait'] !== 'true') {
            ...
            // delay 1 min
            await fgtAutoscaleAws.AutoScaleCore.Functions.sleep(60000);
        }
        // bring up the rest instances which will become the secondary instance(s)
        await autoscaleHandler.updateCapacity(
            autoscaleHandler._settings['byol-scaling-group-name'],
            desiredCapacityBYOL,
            minSizeBYOL,
...
exports.AutoScaleCore = fgtAutoscaleAws.AutoScaleCore;
exports.init = init;
