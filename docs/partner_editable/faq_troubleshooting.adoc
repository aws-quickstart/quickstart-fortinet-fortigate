...
*A.* If the FortiGate master election is not successful, reset the master election. If the reset does not solve the problem, contact support.

*Q.* I want to reset the master election.

*A.* To reset the master election, navigate to the DynamoDB table *<ResourceTagPrefix>-FortiGateMasterElection.* Choose the *Items* tab, and delete the master record (the only item listed). A new master FortiGate is elected, and a new record is created.
