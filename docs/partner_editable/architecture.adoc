...
** FortiGate instances, which act as network address translation (NAT) gateways, allowing outbound internet access for resources in the private subnets. These instances include the following:

*** A FortiGate main instance in an Auto Scaling group.

*** A FortiGate host in an Auto Scaling group. This host complements AWS security groups (not shown), providing intrusion protection, web filtering, and threat detection to help protect your services from cyberattacks. It also allows VPN access by authorized users.
