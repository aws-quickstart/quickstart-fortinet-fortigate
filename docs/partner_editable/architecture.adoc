...
** An Amazon EC2 Auto Scaling group that contains a FortiGate main instance of the BYOL licensing model.

** An Amazon EC2 Auto Scaling group that contains a FortiGate host of the on-demand licensing model. This group complements security groups (not shown). This host provides intrusion protection, web filtering, and threat detection to help protect your services from cyberattacks. It also allows VPN access by authorized users.
